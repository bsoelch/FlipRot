...
uint64_t heapRead(Heap heap, uint64_t addr, ErrorCode* err){
	...
	if(index<SIZE_MAX){
		*err=NO_ERR;
		return heap.sections[index].data[addr-heap.sections[index].start];
	}
...
ErrorCode heapWrite(Heap heap, uint64_t addr, uint64_t data){
	size_t index=sectionIndex(heap,addr);
	if(index<SIZE_MAX){
		heap.sections[index].data[addr-heap.sections[index].start]=data;
		return NO_ERR;
	}
...
static ErrorCode createSection(Heap* heap, uint64_t size){
	...
	heap->sections[heap->len].data=malloc(size*sizeof(uint64_t));
